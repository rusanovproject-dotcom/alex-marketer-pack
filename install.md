...
**JTBD-распаковке** — 13 шагов на каждый сегмент × 3-5 сегментов:

  Big Job → jobStory → Точки А/Б → граф работ → Consideration Set
  → барьеры → Entry/Monetization Job → оценка → ранжирование
  → механики ценности → стратегические гипотезы

На выходе — единый документ `JTBD_анализ_<проект>.md` со всеми сегментами.
Потом отдельный чат `/jtbd-critic` свежим взглядом проверит на абстракции,
противоречия, фактчек.

—

Если есть продающие/диагностические созвоны: «активируй встречи» —
включу модуль `customers/`, парсинг транскриптов в 7 артефактов,
банк возражений.
